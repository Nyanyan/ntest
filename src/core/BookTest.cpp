...
static const bool printTestDebug = false;
...
void TestIsQuestionable(i2 vMover, i2 vOpponent, bool isQuestionable, bool drawToMover=false) {
	assertTrue(vMover>=vOpponent);	// this can't happen in a real book, so if it happens here the test is bogus
	CBookValue bv;
	bv.vMover = vMover;
	bv.vOpponent = vOpponent;
	bool returnedDrawToMover;
	assertTrue(isQuestionable == CBook::IsQuestionable(returnedDrawToMover, drawCutoff, deviationCutoff, bv));
	if (isQuestionable) {
		assertBoolEquals(drawToMover,returnedDrawToMover);
	}
}
